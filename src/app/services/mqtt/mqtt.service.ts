...
export class MQTTService implements TransportService {
  ...
  public configure(config?: Config): void {
    ...
    /*if (typeof this.config.node.address === 'undefined') {
      this.config.node.address = this._document.location.hostname;
    }*/
 }
  ...
  public on_message = (...args: any[]) => {
    ...
    this.debug("onMessage topic", topic);
    this.debug("onMessage message", message.toString());
    //this.debug(packet);

    if (message.toString()) {
      this.messages.next(message.toString());
    } else {
